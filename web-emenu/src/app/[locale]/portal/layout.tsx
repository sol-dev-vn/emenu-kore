    ...
    <div className="min-h-screen flex bg-gray-50 dark:bg-gray-900 transition-colors">
      <aside className={`${isSidebarCollapsed ? 'w-16' : 'w-72'} bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 hidden md:flex flex-col fixed h-full shadow-sm transition-all duration-300 ease-in-out`}>
              ...
              {supportItems.map((item) => {
                ...
                return (
                  <a
                    key={item.href}
                    href={item.href}
                    className={`flex items-center ${isSidebarCollapsed ? 'justify-center' : 'gap-2'} rounded-md px-3 py-2 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white transition-colors`}
        ...
        <div className="p-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
          {/* Branch selector - collapsed shows just icon */}
          {isSidebarCollapsed ? (
            <button
              onClick={() => setShowBranchDialog(true)}
              className="w-full mb-4 p-3 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors flex items-center justify-center"
              title={`Branch: ${selectedBranchName}`}
            >
      ...
      <div className={`flex-1 ${isSidebarCollapsed ? 'md:ml-16' : 'md:ml-72'} transition-all duration-300 ease-in-out`}>
        {children}
      </div>
